...
import (
	...
	"github.com/awkshwayrd/prolog/internal/auth"
	"github.com/awkshwayrd/prolog/internal/config"
	"github.com/awkshwayrd/prolog/internal/log"
...
func TestServer(t *testing.T) {
	for scenario, fn := range map[string]func(
		...
		"unauthorized falis":                                 testUnauthorized,
	} {
...
func testProduceConsumeStream(t *testing.T, client, _ api.LogClient, config *Config) {
	...
	}}
	{
		...
		stream, err := client.ProduceStream(ctx, req)
		require.NoError(t, err)
		for offset, record := range records {
			err = stream.SendMsg(&api.ProduceRequest{
				Record: record,
			},
			)
			require.NoError(t, err)
